...
設定例では `cowrie-host` の `/opt/cowrie/var/log/cowrie/audit.log` に対して、次のような「抽出文字列の集合」を比較します。ホスト名とパスは環境に合わせて変更してください。

```sh
grep -oE '(CMD|Saved|SFTP).+$' /opt/cowrie/var/log/cowrie/audit.log | sort | uniq
```

- 初回は現在値を保存するだけです。既存分も通知したければ `notify_initial: true` にします。
- 2回目以降は追加行を `+`、削除行を `-` でDiscordに通知します。順序変更や同じ行の再出現・回数増加では通知しません。
- SSH先では `LC_ALL=C grep -a -oP` を実行し、Python側で重複を除きます。現在の正規表現エンジンはGNU grepのPCRE（`-P`）です。元のコマンドのERE（`-E`）とは異なります。Cロケールと、バイナリ扱いを避ける `-a` も指定します。
- 通知がすべて成功したチェックだけスナップショットを更新します。SSH・grep・通知の失敗は終了コード1、通常終了・重複起動スキップは0です。マッチ0件は正常な空集合、ファイル不在・権限不足・不正な正規表現はエラーとして区別します。
...
この方式はバイナリ自体のハッシュ比較ではなく、Saved/SFTPを含むログ文字列の差分検知です。
また現在のログを毎回読み直すため、ローテーション・切り詰めでは削除通知が出ます。
`notify_removed: false` なら削除だけの通知を抑止できますが、削除後の再出現は追加扱いです。
5分の間に書かれてローテーション等で監視ファイルから消えた行は検知できません。
...
- 判定は「現在時刻 − 前回の実行開始時刻 ≥ interval_seconds」です。経過していないcheckはSSH接続せずスキップし、残り秒数をログに記録します。
...
- 間隔だけを変更すると、前回の開始時刻を基準に新しい間隔を適用します。ホスト・ファイル・正規表現の変更や新しいcheckは即実行します。
- `--dry-run` も同じ間隔判定を行いますが、実行時刻も比較用データも更新しません。
- 保存済みの開始時刻より時計が戻った場合は即実行し、新しい時刻を基準にします。時刻はUTCのUnix秒なのでタイムゾーン変更に依存しません。

`config.json` の `checks` 配列に一意の `name` を持つ要素を追加します。例えば下記は **ログ形式を仮定した例** です。実際のログに合わせて正規表現を調整してください。

```json
{
  "name": "source-addresses",
  "interval_seconds": 3600,
  "file": "/path/to/other.log",
  "regex": "src_ip=[0-9.]+",
  "notify_removed": false
}
```

タイムスタンプや接続IDを抽出に含めると、それらが変わるたびに新しい値と判定されます。
IP・ユーザー名など比較したい部分だけをマッチさせてください。PCREの後読みも使用できます。`-o` はキャプチャグループだけでなくマッチ全体を返します。
JSONの中ではバックスラッシュを `\\` と書きます。

`name` を変更した場合や、SSHホスト・ファイル・正規表現を変更した場合、そのチェックは新しい基準値を作ります。
通知設定だけの変更では基準値を保持します。状態ファイルを消すと全チェックが初回扱いになります。
壊れた状態ファイルは自動で上書きせずエラーにします。
チェック単位で状態を持つため、一つが失敗しても他のチェックは実行します。
...
スクリプト・設定・`pyproject.toml`・`uv.lock` を `/opt/cowrie-discord/` に配置し、実行ユーザーがプロジェクト内の仮想環境・ログ・状態ディレクトリを書けるようにします。
...
JSONは一度 `object` として受け取り、実行時に検証してから型付きデータクラスに変換します。設定・状態ファイルの形式は従来のままです。
